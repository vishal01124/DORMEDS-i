...
const A = {
  ...
  payBill(id){
    ...
    const body='<div style="text-align:center;padding:4px 0 10px">'
      ...
      +'<div style="margin:10px 0 8px;font-size:.85rem;color:var(--txt2)">Scan QR or tap <strong style="color:var(--txt)">Open UPI App</strong> in footer to pay</div>'
      +'<div style="margin-bottom:12px;background:var(--inp);border-radius:8px;padding:8px 14px;display:inline-flex;align-items:center;gap:10px"><div><div style="font-size:.68rem;color:var(--mute);margin-bottom:2px">UPI ID</div><div style="font-family:monospace;font-weight:700;color:var(--acc)">'+upi+'</div></div><button class="btn btn-sm btn-s" onclick="navigator.clipboard.writeText(\''+upi+'\').then(()=>A.toast(\'Copied!\',\'ok\'))"><span class="material-icons-round" style="font-size:16px">content_copy</span></button></div></div>'
      ...
      +'<button class="btn btn-ok" style="width:100%;justify-content:center;margin-top:4px" onclick="A.submitUTR('+JSON.stringify(id)+')"><span class="material-icons-round">send</span>Submit UTR</button>';
    const foot='<button class="btn btn-s" onclick="A.closeModal()">Cancel</button>'
      +'<a href="'+upiUrl+'" class="btn btn-p" style="text-decoration:none;background:linear-gradient(135deg,#00D48E,#1a73e8);font-weight:700"><span class="material-icons-round">open_in_new</span>Open UPI App</a>';
